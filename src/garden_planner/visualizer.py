...
def default_theme() -> Theme:
    ...
    styles = {
        "snowdrop": SpeciesStyle(label="Snowdrops (Galanthus elwesii)", spacing=7,  bulbs=40, size=140, face="#2e8b57", edge="#2e8b57"),
        "crocus_mixed": SpeciesStyle(label="Crocus tommasinianus", spacing=7, bulbs=25, size=110, face="#800000", edge="#800000"),
        "scilla": SpeciesStyle(label="Scilla siberica", spacing=8, bulbs=20, size=120, face="#808000", edge="#808000"),
        "daffodil": SpeciesStyle(label="Narcissus 'Tête-à-Tête'", spacing=10, bulbs=30, size=150, face="#663399", edge="#663399"),
        "iris_reticulata": SpeciesStyle(label="Iris reticulata", spacing=8, bulbs=15, size=140, face="#ff0000", edge="#ff0000"),
        "hyacinth": SpeciesStyle(label="Hyacinth 'Jan Bos'", spacing=12, bulbs=5, size=180, face="#ff8c00", edge="#ff8c00"),
        "chionodoxa": SpeciesStyle(label="Chionodoxa luciliae alba", spacing=8, bulbs=20, size=120, face="#ffd700", edge="#ffd700"),
        "anemone_blanda": SpeciesStyle(label="Anemone blanda", spacing=8, bulbs=25, size=130, face="#0000cd", edge="#0000cd"),
        "tulip_mixed": SpeciesStyle(label="Tulip Triumph (mixed colors)", spacing=12, bulbs=50, size=160, face="#ff1493", edge="#ff1493"),
        "tulip_ballerina": SpeciesStyle(label="Tulip 'Ballerina'", spacing=12, bulbs=7, size=160, face="#adff2f", edge="#adff2f"),
        "iris_hollandica": SpeciesStyle(label="Iris hollandica 'Red Ember'", spacing=12, bulbs=10, size=150, face="#1e90ff", edge="#1e90ff"),
        "anemone_de_caen": SpeciesStyle(label="Anemone de Caen", spacing=8, bulbs=20, size=130, face="#fa8072", edge="#fa8072"),
        "tulip_white": SpeciesStyle(label="Tulip Triumph (white)", spacing=12, bulbs=25, size=160, face="#eee8aa", edge="#eee8aa"),
        "tulip_rembrandt": SpeciesStyle(label="Tulip Rembrandt (mixed)", spacing=12, bulbs=15, size=160, face="#dda0dd", edge="#dda0dd"),
    }
...
HEIGHTS_CM: Dict[str, int] = {
    "snowdrop": 15, "crocus_mixed": 12, "scilla": 15,
    "chionodoxa": 12, "iris_reticulata": 14, "anemone_blanda": 12,
    "daffodil": 22, "hyacinth": 25, "anemone_de_caen": 22,
    "tulip_mixed": 45, "tulip_white": 45, "tulip_rembrandt": 45,
    "tulip_ballerina": 60, "iris_hollandica": 60,
}
...
def legend_handles(clusters: Dict[str, List[Tuple[float, float, int]]], theme: Theme) -> List[mpatches.Patch]:
...
def total_halo_area(species_clusters: Dict[str, List[Tuple[float,float,int]]], theme: Theme) -> float:
...
def camera_homography(bed: GardenBed, cam_height=1.55, cam_dist=0.30, cam_pitch_deg=35.0, cam_focal=1.8):
    """
    ...
    R_x = np.array([[1, 0, 0],
                    ...
                    [0, -np.sin(th),  np.cos(th)]], dtype=float)
...
def make_non_overlapping_layout(species_clusters: Dict[str, List[Tuple[float,float,int]]],
                                theme: Theme, bed: GardenBed,
                                gap: float = 0.02, max_iters: int = 800, step: float = 0.6,
            ...
            for i2 in range(n):
                for j2 in range(i2+1, n):
                    ...
                    if overlap > 0:
                        ...
                        items[i2]["x"] -= ux * push; items[i2]["y"] -= uy * push
                        items[j2]["x"] += ux * push; items[j2]["y"] += uy * push
            for obj in items:
                x, y, r = obj["x"], obj["y"], obj["r"]
                obj["x"] = min(max(r, x), bed.width_m - r)
                obj["y"] = min(max(r, y), bed.depth_m - r)
        ...
        for i in range(n):
            for j in range(i+1, n):
                ...
                if overlap > 0:
                    ...
                    items[i]["x"] -= ux * push; items[i]["y"] -= uy * push
                    items[j]["x"] += ux * push; items[j]["y"] += uy * push
        for obj in items:
            x, y, r = obj["x"], obj["y"], obj["r"]
            obj["x"] = min(max(r, x), bed.width_m - r)
            obj["y"] = min(max(r, y), bed.depth_m - r)
...
def autoscale_to_density(bed: GardenBed, clusters: Dict[str, List[Tuple[float,float,int]]],
                         theme: Theme, density: Optional[float],
                         scale_min: float = 0.3, scale_max: float = 3.0,
                         repack_gap: float = 0.02, max_iters: int = 800, step: float = 0.6):
    """
    ...
    scaled = {sp: [(x*s_clamped, y*s_clamped, c) for (x,y,c) in pts] for sp, pts in clusters.items()}
    repacked = make_non_overlapping_layout(scaled, theme, new_bed, gap=repack_gap, max_iters=max_iters, step=step, interleave_species=False)
...
def _fit_axes_to_points(ax, pts, margin=0.05):
    ...
    dx = max(1e-9, xmax - xmin); dy = max(1e-9, ymax - ymin)
...
# ---------------------- Rendering ----------------------
def render_schematic(title: str, clusters: Dict[str, List[Tuple[float, float, int]]],
                     bed: GardenBed, theme: Theme,
                     cell_size: float = CELL_SIZE, show_halos: bool = False, halo_alpha: float = None,
                     save_path: str = None, show_legend: bool = True):
    fig, ax = plt.subplots(figsize=(11, 7))
    ax.set_xlim(0, bed.width_m); ax.set_ylim(0, bed.depth_m)
    ...
    for sp, pts in clusters.items():
        ...
        for (x, y, c) in pts:
            if show_halos:
                ...
                circ = plt.Circle((x, y), r, facecolor=st.face,
                                  edgecolor=st.edge, alpha=halo_alpha, lw=1)
                ax.add_patch(circ)
            ax.scatter(x, y, facecolors=st.face, edgecolors=st.edge,
                       s=st.size, marker=st.marker, zorder=3)
            _annotate(ax, x + 0.03, y + 0.03, str(c), st.face, 9)

    if show_legend:
        ax.legend(handles=legend_handles(clusters, theme), bbox_to_anchor=(1.02, 1), loc="upper left")
    plt.tight_layout()
    if save_path:
...
def render_perspective_grid_camera(title: str, clusters, bed: GardenBed, theme: Theme,
                                   H, sign_v, cell_size=CELL_SIZE,
                                   outline_color="gray", grid_color="lightgray",
                                   show_halos=False, halo_alpha=None, save_path=None,
                                   show_legend: bool = True):
    fig, ax = plt.subplots(figsize=(11,7))
    ax.set_facecolor("white"); ax.set_title(title + "\nTrue camera perspective (grid)")
    ...
    n_x = int(round(bed.width_m / cell_size)); n_y = int(round(bed.depth_m / cell_size))
    ...
    for i in range(n_x + 1):
        ...
        _annotate(ax, float(dst_pt[0]) + label_offset, float(dst_pt[1]) - label_offset, _col_letters(i), "black", 9)
    # Rows along left edge
    for j in range(n_y + 1):
        y = j * cell_size
        src_pt = np.array([[0.0, y]])
        dst_pt = apply_homography(H, src_pt, sign_v)[0]
        _annotate(ax, float(dst_pt[0]) - 2*label_offset, float(dst_pt[1]) + label_offset, str(j + 1), "black", 9)
    ...
    if show_halos:
        for sp, pts in clusters.items():
            ...
            for (x,y,c) in pts:
                ...
                ax.fill(halo[:,0], halo[:,1], facecolor=st.face, edgecolor=st.edge,
                        alpha=halo_alpha, lw=1, zorder=1)
                all_pts.append(halo)
    ...
    for sp, pts in clusters.items():
        ...
        for (Xp, Yp), (_,_,c) in zip(dst, pts):
            ax.scatter(Xp, Yp, marker=st.marker, facecolors=st.face,
                       edgecolors=st.edge, s=st.size, zorder=4)
...
def render_perspective_heights_camera(title: str, clusters, bed: GardenBed, theme: Theme,
                                      H, sign_v,
                                      front_scale=0.55, back_scale=0.25,
                                      outline_color="gray", show_halos=True, halo_alpha=None,
                                      save_path=None, show_legend: bool = True):
    fig, ax = plt.subplots(figsize=(11,7))
    ...
    for sp, pts in clusters.items():
        ...
        for (Xp, Yp), (x,y,c) in zip(base, pts):
            h_m = HEIGHTS_CM.get(sp, 20)/100.0
            dY = h_m * scale_at_depth(y)
            topY = Yp + dY
            ax.plot([Xp, Xp], [Yp, topY], color=st.face, lw=2, zorder=3)
            ax.scatter(Xp, topY, marker=st.marker, facecolors=st.face,
                       edgecolors=st.edge, s=st.size/2, zorder=4)
...
# ---------------------- CSV Export / Import ----------------------
def export_plan_csv(clusters: Dict[str, List[Tuple[float,float,int]]], bed: GardenBed, theme: Theme, path: str):
    rows = []
    for sp, pts in clusters.items():
        for (x,y,c) in pts:
            spacing = theme.species_styles[sp].spacing
            ...
            rows.append({"species_key": sp, "species_label": theme.species_styles[sp].label,
                         "x_m": x, "y_m": y, "bulbs_in_cluster": c,
                         "spacing_cm": spacing, "area_m2": A, "equiv_radius_m": r,
                         "bed_width_m": bed.width_m, "bed_depth_m": bed.depth_m})
    pd.DataFrame(rows).to_csv(path, index=False)


def import_plan_csv(path: str) -> Tuple[Dict[str, List[Tuple[float,float,int]]], Optional[float], Optional[float]]:
    df = pd.read_csv(path)
    clusters: Dict[str, List[Tuple[float,float,int]]] = {}
    ...
    for _, row in df.iterrows():
        ...
        x = float(row["x_m"]); y = float(row["y_m"]); c = int(row["bulbs_in_cluster"])
...
def filter_clusters_by_month(clusters: Dict[str, List[Tuple[float,float,int]]], month_name: str) -> Dict[str, List[Tuple[float,float,int]]]:
    month_map = {
...
def main():
    ...
    parser.add_argument("--interleave-species", action="store_true", help="Ensure clusters of the same species are not direct neighbors; a different species must lie between any same-species pair along their line of centers.")
    parser.add_argument("--interleave-validate-strict", action="store_true", help="Extra sure interleaving yeeeee")
    ...
    parser.add_argument("--density", type=float, default=0.5, help="Target coverage 0..1 for auto-scaling the bed")
    parser.add_argument("--scale-min", type=float, default=0.3, help="Minimum allowed uniform scale")
    parser.add_argument("--scale-max", type=float, default=3.0, help="Maximum allowed uniform scale")

    # load from CSV and month filter
    parser.add_argument("--from-csv", type=str, default=None, help="Path to a previously exported plan CSV")
    parser.add_argument("--month", type=str, default=None, help="Month name to filter species by bloom time (requires --from-csv)")
    parser.add_argument("--bed-width", type=float, default=None, help="Override bed width when loading from CSV without metadata")
    parser.add_argument("--bed-depth", type=float, default=None, help="Override bed depth when loading from CSV without metadata")
    ...
    if args.from_csv:
        ...
        # Camera
        H, sign_v = camera_homography(bed, cam_height=args.cam_height, cam_dist=args.cam_dist,
                                      cam_pitch_deg=args.cam_pitch, cam_focal=args.cam_focal)
        ...
        render_schematic(f"Filtered View — {args.month or 'All'}", clusters, bed, theme,
                         cell_size=args.cell, show_halos=args.halos,
                         save_path=f"{out_prefix}_schematic.png",
                         show_legend=not args.legend_off)

        render_perspective_grid_camera(f"Filtered View — {args.month or 'All'}", clusters, bed, theme, H, sign_v,
                                       cell_size=args.cell, show_halos=args.halos,
                                       save_path=f"{out_prefix}_perspective_grid.png",
                                       show_legend=not args.legend_off)

        render_perspective_heights_camera(f"Filtered View — {args.month or 'All'}", clusters, bed, theme, H, sign_v,
        ...
        clusters = build_random_clusters(theme=theme, bed=bed, rng=rng, avg=args.avg, min_odd=args.min_odd)
        clusters = make_non_overlapping_layout(clusters, theme, bed, gap=args.gap, max_iters=800, step=0.6, interleave_species=args.interleave_species)

        if args.density is not None:
            s, bed, clusters = autoscale_to_density(bed, clusters, theme, args.density,
                                                    scale_min=args.scale_min, scale_max=args.scale_max,
                                                    repack_gap=args.gap, max_iters=800, step=0.6)
            print(f"[density] target={args.density:.2f}  scale={s:.3f}  new bed = {bed.width_m:.2f}×{bed.depth_m:.2f} m")
        ...
        render_schematic("All_Bulbs_Schematic", clusters, bed, theme,
                         cell_size=args.cell, show_halos=args.halos,
                         save_path=f"{out_prefix}_schematic.png",
                         show_legend=not args.legend_off)

        render_perspective_grid_camera("All_Bulbs_Perspective (Camera Grid)", clusters, bed, theme, H, sign_v,
                                       cell_size=args.cell, show_halos=args.halos,
                                       save_path=f"{out_prefix}_perspective_grid.png",
                                       show_legend=not args.legend_off)

        render_perspective_heights_camera("All_Bulbs_Perspective (Camera Heights)", clusters, bed, theme, H, sign_v,
                                          show_halos=args.halos,
                                          save_path=f"{out_prefix}_perspective_heights.png",
                                          show_legend=not args.legend_off)
